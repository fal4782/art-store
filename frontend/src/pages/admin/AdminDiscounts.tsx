...
export default function AdminDiscounts() {
  ...
  const handleDelete = async (discount: DiscountCode) => {
    if (
      window.confirm(`Are you sure you want to delete code "${discount.code}"?`)
    ) {
      try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="font-bold opacity-40">
            Manage promotional codes and coupons.
          </p>
        </div>
          ...
          {getFilteredDiscounts().length === 0 ? (
            <div className="col-span-full py-20 flex flex-col items-center justify-center text-center space-y-4 opacity-30">
              ...
              <p className="font-bold text-lg">
                No discounts found matching filters.
              </p>
            </div>
          ) : (
            getFilteredDiscounts().map((discount) => (
              <div
                ...
                className={`group relative p-5 rounded-3xl border shadow-sm transition-all duration-300 overflow-hidden ${
                  !discount.isActive
                    ? "bg-stone-50 opacity-60 grayscale hover:grayscale-0 hover:opacity-100"
                    : "bg-white"
                }`}
                style={{
                  borderColor: !discount.isActive ? "transparent" : "#e7e5e4",
                }}
              >
                ...
                <div className="flex justify-between items-start mb-4 relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold tracking-wider-500`}
                        style={{
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-stone-50 w-full">
                    ...
                    >
                      {discount.minPurchaseInPaise
                        ? `₹${discount.minPurchaseInPaise / 100}`
                        : "None"}
                    </span>
                  </div>
                ...
                {discount.description && (
                  <p
                    className="mt-4 text-xs font-medium opacity-60 leading-relaxed italic border-l-2 pl-3"
