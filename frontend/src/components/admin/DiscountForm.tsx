...
import { FiTag, FiPercent, FiCalendar, FiCheck, FiX, FiLoader, FiHash } from "react-icons/fi";
import { theme } from "../../theme";
import { discountService } from "../../services/discountService";
import type { CreateDiscountInput, DiscountCode } from "../../services/discountService";
...
export default function DiscountForm({ onSuccess, onCancel, initialData }: DiscountFormProps) {
  ...
  const [formData, setFormData] = useState<CreateDiscountInput>({
    ...
    minPurchaseInPaise: initialData?.minPurchaseInPaise ? initialData.minPurchaseInPaise / 100 : 0, // Convert paise to rupees for input
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        minPurchaseInPaise: Number(formData.minPurchaseInPaise) * 100 || undefined, // Convert rupees to paise
      ...
      console.error(err);
      const msg = err.response?.data?.message || `Failed to ${initialData ? 'update' : 'create'} discount`;
      showToast(msg, "error");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 animate-fade-in shadow-xl max-w-2xl mx-auto border border-stone-100">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-xl font-black flex items-center gap-3" style={{ color: theme.colors.primary }}>
          <div className="p-3 rounded-xl" style={{ backgroundColor: `${theme.colors.secondary}15`, color: theme.colors.secondary }}>
             <FiTag size={24} />
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="space-y-1 md:space-y-2">
             <label className={labelClass} style={{ color: theme.colors.primary }}>Discount Code</label>
           ...
           {/* Type Select */}
           <div className="space-y-1 md:space-y-2">
             <label className={labelClass} style={{ color: theme.colors.primary }}>Type</label>
             <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           {/* Min Purchase */}
           <div className="space-y-1 md:space-y-2">
             <label className={labelClass} style={{ color: theme.colors.primary }}>Min Purchase</label>
             <div className="relative">
               ...
               <input 
                 ...
                 onChange={e => setFormData({ ...formData, minPurchaseInPaise: Number(e.target.value) })}
                 className={inputClass}
                 style={{ 
                   borderColor: `${theme.colors.accent}40`,
                   backgroundColor: `${theme.colors.accent}15`,
        ...
        <div className="space-y-1 md:space-y-2">
             <label className={labelClass} style={{ color: theme.colors.primary }}>Description (Optional)</label>
             <textarea 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           {/* Max Uses */}
           <div className="space-y-1 md:space-y-2">
             <label className={labelClass} style={{ color: theme.colors.primary }}>Max Uses</label>
             <div className="relative">
           ...
           {/* Valid Until */}
           <div className="space-y-1 md:space-y-2">
             <label className={labelClass} style={{ color: theme.colors.primary }}>Expires On</label>
             <div className="relative">
