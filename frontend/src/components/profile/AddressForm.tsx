...
export default function AddressForm({
  ...
  const iconClass = "absolute left-4 top-1/2 -translate-y-1/2 text-xl transition-colors duration-300 opacity-40";
  ...
  return (
    <div className="p-8 md:p-10 rounded-[2.5rem] bg-white border border-stone-100 shadow-sm animate-scale-in relative overflow-hidden group">
      ...
      <div className="absolute -top-24 -right-24 w-64 h-64 rounded-full opacity-[0.03] pointer-events-none group-hover:scale-110 transition-transform duration-700" 
        style={{ backgroundColor: theme.colors.secondary }} />

      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-2xl" style={{ backgroundColor: `${theme.colors.secondary}15`, color: theme.colors.secondary }}>
            <FiMapPin size={24} />
          </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <label className={labelClass} style={{ color: theme.colors.primary }}>Full Name</label>
          <div className="relative">
            <FiUser className={iconClass} style={{ color: theme.colors.primary, opacity: isFocused === 'name' ? 1 : 0.4 }} />
            <input 
              required 
              value={formData.name} 
        ...
        {/* Address Line 1 */}
        <div className="space-y-2">
          <label className={labelClass} style={{ color: theme.colors.primary }}>Address Line 1</label>
          <div className="relative">
            <FiMapPin className={iconClass} style={{ color: theme.colors.primary, opacity: isFocused === 'line1' ? 1 : 0.4 }} />
            <input 
              required 
        ...
        {/* Address Line 2 */}
        <div className="space-y-2">
          <label className={labelClass} style={{ color: theme.colors.primary }}>Address Line 2 (Optional)</label>
          <div className="relative">
            <FiMapPin className={iconClass} style={{ color: theme.colors.primary, opacity: isFocused === 'line2' ? 1 : 0.4 }} />
            <input 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              <FiMapPin className={iconClass} style={{ color: theme.colors.primary, opacity: isFocused === 'city' ? 1 : 0.4 }} />
              <input 
                required 
                value={formData.city} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className={labelClass} style={{ color: theme.colors.primary }}>State</label>
            <div className="relative">
              <FiMapPin className={iconClass} style={{ color: theme.colors.primary, opacity: isFocused === 'state' ? 1 : 0.4 }} />
              <input 
          ...
          <div className="space-y-2">
            <label className={labelClass} style={{ color: theme.colors.primary }}>Phone</label>
            <div className="relative">
              <FiPhone className={iconClass} style={{ color: theme.colors.primary, opacity: isFocused === 'phone' ? 1 : 0.4 }} />
              <input 
        ...
        <div className="flex items-center gap-4 py-4 px-2 group/cb">
          <div className="relative flex items-center">
            <input 
              ...
              style={{ 
                backgroundColor: formData.isDefault ? theme.colors.secondary : 'transparent',
                borderColor: isFocused === 'checkbox' 
                  ? `${theme.colors.primary}40` 
                  : (formData.isDefault ? 'transparent' : `${theme.colors.primary}20`)
              }}
          ...
          <div className="flex flex-col">
            <label htmlFor="isDefault" className="font-bold cursor-pointer text-sm select-none transition-colors" style={{ color: theme.colors.primary }}>
              Set as default shipping address
            </label>
            <span className="text-[10px] font-bold opacity-40 uppercase tracking-widest">Primary address for all future orders</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-end gap-4 pt-6 border-t border-stone-100">
          ...
          <button 
            ...
            style={{ background: theme.colors.secondary, boxShadow: `0 10px 30px -10px ${theme.colors.secondary}60` }}
          >
            {isLoading ? (
