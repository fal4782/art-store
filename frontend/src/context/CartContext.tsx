...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const clearCart = useCallback(async () => {
    try {
      ...
      showToast(
        "Order placed but failed to clear cart. Please refresh.",
        "error",
      );
  ...
  const addToCart = useCallback(
    async (artworkId: string, quantity: number = 1) => {
      if (!isAuthenticated) {
  ...
  const updateQuantity = useCallback(
    async (itemId: string, quantity: number) => {
      ...
      if (item && quantity > item.artwork.stockQuantity) {
        showToast(
          `Only ${item.artwork.stockQuantity} units available`,
          "error",
        );
        return;
      }
  ...
  const totalAmount = cart.reduce(
    (total, item) => total + item.artwork.priceInPaise * item.quantity,
    0,
  );
