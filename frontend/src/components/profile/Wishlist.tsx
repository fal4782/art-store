...
export default function Wishlist() {
    ...
    return (
      <div className="max-w-4xl space-y-8 animate-pulse">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-80 rounded-[32px] bg-stone-50 border border-stone-100" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl space-y-10 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-black mb-2" style={{ color: theme.colors.primary }}>My Wishlist</h1>
        <p className="opacity-60 font-medium">Your curated collection of favorite pieces.</p>
      </div>

      {wishlist.length === 0 ? (
        <div className="py-20 text-center space-y-6 bg-white rounded-[40px] border border-stone-100 shadow-sm">
          <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto opacity-10" style={{ backgroundColor: theme.colors.primary }}>
            <FiHeart size={40} />
          </div>
          <div className="space-y-2">
            <h2 className="text-xl font-black" style={{ color: theme.colors.primary }}>Your Wishlist is Empty</h2>
            <p className="opacity-60 max-w-sm mx-auto">Looks like you haven't saved any masterpieces yet. Explore the shop to find something you love!</p>
          </div>
          <Link 
            to="/shop" 
          ...
          {wishlist.map((item) => (
            ...
            >
              ...
              >
                ...
                />
                <button 
                  onClick={(e) => { e.stopPropagation(); toggleWishlist(item.artwork.id); }}
              ...
              <div className="p-6 space-y-4">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    <h3 className="font-black text-lg line-clamp-1" style={{ color: theme.colors.primary }}>
                      {item.artwork.name}
                    </h3>
                  ...
                  <span className="font-black text-lg" style={{ color: theme.colors.primary }}>
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    style={{ borderColor: `${theme.colors.primary}10`, color: theme.colors.primary }}
                  >
                    View Details
                  </button>
                  <button 
